...
def setJobType(String suiteInfo) {
    ...
    switch(suiteInfo) {
        case ~/^(?!.*web).*api.*$/:
            ...
            return setJobParameters("env,url", "API", "spot-fleet")
        case ~/^.*web.*$/:
        ...
        default:
            ...
            return setJobParameters("env,url", "*", "spot-fleet")
    }
}
...
def prepare(Map jobParameters) {
    stage('Preparation') {
        currentBuild.displayName = "#${BUILD_NUMBER}|${suite}|${env.env}|${CARINA_CORE_VERSION}|${device}"
